...
async function fetchRepliersStatus(apiKey, standardStatus, polygon, { resultsPerPage = 200, maxPages = 5, boardId } = {}) {
  const map = polygonToRepliersMap(polygon);
  let listings = [];
  for (let page = 1; page <= maxPages; page++) {
    // Repliers only accepts `map` in the POST body; every other filter must be
    // a query-string parameter (body params are returned as unrecognizedParams).
