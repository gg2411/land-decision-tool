...
function summarizeComps(listings, { minSqft, maxSqft, minBeds, minYearBuilt, soldWithinMonths, now } = {}) {
  ...
  const filtered = listings.filter((l) => {
    ...
    if (minYearBuilt && l.yearBuilt != null && l.yearBuilt < minYearBuilt) return false;
...
function splitLotScenario(totalLotPrice, totalLotSqft, plan, comps, costs, nSplits = 2) {
  ...
  return {
    ...
    impliedMarginPerLot: impliedMargin(plan, comps, costs, perLotPrice).marginPct,
...
async function fetchRepliersStatus(apiKey, standardStatus, polygon, { resultsPerPage = 200, maxPages = 5, boardId, minSoldDate } = {}) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    const body = { map, standardStatus, resultsPerPage, pageNum: page };
    if (boardId) body.boardId = boardId;
    if (standardStatus === "Closed" && minSoldDate) body.minSoldDate = minSoldDate;

    const resp = await fetch(`${BASE_URL}/listings`, {
      method: "POST",
      headers: {
...
function parseListing(rec) {
  ...
  return {
    ...
    pricePerSqft: sqft && recPrice(rec) ? round2(recPrice(rec) / sqft) : null,
    latitude: geo.latitude,
    longitude: geo.longitude,
    address: [addr.streetNumber, addr.streetName].filter(Boolean).join(" "),
    city: addr.city,
  };
}

function recPrice(rec) {
  const closePrice = rec.soldPrice != null ? Number(rec.soldPrice) : null;
  const listPrice = rec.listPrice != null ? Number(rec.listPrice) : null;
