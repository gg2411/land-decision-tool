...
const LINK = "of|was|is|at|averages?|around|about";
// "median price per square foot of $513" / "per square foot was $556.75"
const PPSF_AFTER_RE = new RegExp(
  // unit, then either: the amount directly / after punctuation; a bare linker
  // ("was $513"); or a short qualifier starting with of/for/in ("for homes sold
  // in 77005, Houston, TX,") followed by a linker. Never crosses `$ ; .`
  `(?:per|/)\\s*(?:sq\\.?\\s?ft|square\\s?foot|sqft)` +
    `(?:\\s*[:\\-–—]?|\\s+(?:${LINK})|\\s+(?:of|for|in)\\b[^$;.]{0,60}?(?:\\b(?:${LINK})\\b|[:\\-–—]))` +
    `\\s*\\$\\s?${NUM}`,
  "gi"
);
...
function summarizeWebResults(results, answer) {
  ...
  for (const r of results || []) {
    ...
    if (!ppsf.length) continue; // figure-less page must not block a later page on the same host
    seenHosts.add(host);
  ...
  if (answerPpsf.length) {
    ...
    // answer values feed the stats only when the results gave nothing —
    // they typically restate figures already counted per-source.
    if (!all.length) all.push(...answerPpsf);
  }
...
async function tavilySearch(query, apiKey, timeoutMs, extra = {}) {
...
module.exports = { fetchWebPpsf, extractPpsf, summarizeWebResults, tavilySearch };
