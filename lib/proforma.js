...
const DEFAULT_COSTS = {
  ...
  targetMarginPct: 0.15, // profit / total cost
};
...
function specProForma({ salePrice, sqft, lotPrice, costs = {} }) {
  ...
  return {
    ...
    allInCostPerSqft: sqft > 0 ? r2(totalCost / sqft) : null,
    ...
    verdict: verdictFor(r4(marginPct), c.targetMarginPct, profit),
  };
}

function verdictFor(marginPct, targetMarginPct, profit) {
  if (marginPct >= targetMarginPct) return "go";
...
function maxLotPrice({ salePrice, sqft, costs = {} }) {
  ...
  // profit = targetMargin * totalCost  and  profit = sale*(1-sell) - totalCost
  const totalCost = (salePrice * (1 - c.sellingCostPct)) / (1 + c.targetMarginPct);
