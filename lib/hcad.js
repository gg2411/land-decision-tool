...
async function fetchHcadParcels(polygon, { maxPages = 5, pageSize = 1000, timeoutMs = 20000 } = {}) {
...
function summarizeLandValues(parcels, { soldWithinMonths, now } = {}) {
  ...
  const months = soldWithinMonths || 24;
  const cutoff = new Date((now || new Date()).getTime() - months * 30.44 * 86400000);
  const recent = eligible.filter((p) => p.transferDate && new Date(p.transferDate) >= cutoff);

  const used = recent.length >= 5 ? recent : eligible;
  const basis = recent.length >= 5 ? "recent-transfers" : "all-parcels";
